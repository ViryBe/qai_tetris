...
Pour augmenter les performances de l'agent, le modèle peut être affine, par
exemple en considérant plus de cases.

Le nombre d'états étant le nombre de lignes de la Q matrice, son ordre de
grandeur doit rester limite (dans notre cas, environ 20000) pour des
...
nombre d'états, il n'est pas sur qu'elle soit plus performante (apprentissage
...
\paragraph{Creation de faisceaux}
...
\paragraph{Parametres optimaux}
Les parametres \(\alpha, \gamma, \epsilon\) optimisant au choix la hauteur
finale, l'ecart type ou la vitesse de convergence sont determines par un script
bash donnant des donnees a traiter a un script python.
...
L'implémentation actuelle peut voir sa représentation des états allégée en
tirant profit de la symétrie du plateau, chaque état dispose d'un état
symétrique par rapport a un axe au milieu du plateau. On divise ainsi le nombre
d'états par deux. On peut ainsi espérer une convergence plus rapide de la
matrice Q vers la matrice optimale.

D'autres méthodes d'apprentissages pourraient également être explorées. On peut
